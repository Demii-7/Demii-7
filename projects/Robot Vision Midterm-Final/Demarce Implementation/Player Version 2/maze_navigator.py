...
def localize_robust(query_fpv: np.ndarray,
    ...
    """
    Multi-crop voting localisation with graph-aware temporal consistency.

    Splits the FPV into 4 overlapping crops, encodes each, and votes on
    the best matching keyframe. Adjacent-node boost applied when graph and
    current_node are provided.

    FIX [2]: Temp files created for encode() calls are now deleted inside
    a try/finally block so they are cleaned up even if encode() raises.
    """
    ...
    regions = [
        pil,
        pil.crop((w // 4, h // 4, 3 * w // 4, 3 * h // 4)),
        pil.crop((0,      0,      w // 2,      h // 2)),
        pil.crop((w // 2, 0,      w,           h // 2)),
    ]

    votes = np.zeros(len(keyframes))
    ...
    for region in regions:
        ...
        # FIX [2]: always delete temp file, even on encode() exception
        try:
            q = encoder.encode(tmp_path).reshape(1, -1).astype(np.float32)
        finally:
            os.unlink(tmp_path)

        scores, idxs = index.search(q, 10)
        for score, idx in zip(scores[0], idxs[0]):
            if 0 <= idx < len(keyframes):
                boost = 1.0
                if current_node is not None and graph is not None:
                    if graph.has_edge(current_node, idx) or current_node == idx:
                        boost = 1.20
                votes[idx] += float(score) * boost

    best_node = int(np.argmax(votes))
    return best_node, float(votes[best_node])
...
class MazeNavigator:
    ...
    def next_action(self) -> str:
        ...
        Returns 'stop' when the goal is reached or no path is set.
        Returns 'use_radar' if the edge is a loop closure, delegating
        the physical movement decision to the visual scanner.
        Called every frame from player.py act().
        """
        ...
        # THE HANDOFF: If it's a wormhole, tell the player to use its eyes
        if edge.get("edge_type") == "loop_closure" or edge.get("action") == "loop":
            return "use_radar"
            
        # Otherwise, trust the recorded optical flow memory
    ...
    def scan_directions(self, fpv: np.ndarray,
        ...
        """
        Compare 4 directional crops to nodes in the planned route.
        ...
        """
        if not isinstance(self.current_path, list):
            print(f"[navigator] WARNING: current_path is "
                  f"{type(self.current_path).__name__}, not list — resetting")
            self.current_path = []
        ...
        if path_segment is None:
            # FIX: Look ahead! Skip index 0 (the current node) so we don't match 
            # against where we already are. Grab an extra node to maintain depth.
            segment_len  = min(7, len(self.current_path))
            path_segment = list(self.current_path[1:segment_len])

        crops   = self.extract_directional_crops(fpv)
        results = {}
        ...
        for direction, crop in crops.items():
            ...
            # FIX: Start enumerate at 1 so the distance accurately reflects 
            # the offset from the current node.
            ...
            if best_node is not None:
                reported_distance = (
                    -best_distance if direction == "back" else best_distance
                )
                results[direction] = {
                    "node":     best_node,
                    "score":    best_score,
    ...
    def detect_junction(self, direction_scores: dict,
        ...
        """
        Detect if multiple directions have strong alignment (junction).

        Returns True if ≥ 2 directions score above threshold, indicating
        the robot is at a choice point.
        """
        strong_matches = sum(
            1 for v in direction_scores.values()
