...
def detect_turns_single_frame(prev_gray: np.ndarray,
                               ...
                               last_turn: int,
                               min_gap: int = 10,
                               save_dir: str = "/tmp/kf_frames") -> dict | None:
    """
...
def build_pose_graph(
    ...
    Called once in pre_navigation() after build_descriptor_index().
    """
    G = nx.DiGraph()
    N = len(keyframes)
    ...
    lc_count = 0
    all_scores, all_idxs = index.search(descriptors, 15)  # shape: (N, 15)

    for i, (scores, idxs) in enumerate(zip(all_scores, all_idxs)):
        for score, j in zip(scores, idxs):  # ← correct: scores and idxs are already 1D rows
            if j == i or j < 0:
                continue
            gap = abs(
