...
class KeyboardPlayerPyGame(Player):
    def __init__(self):
        ...
        # ── PD controller + escape state ──────────────────────────────────
        self._prev_e_k        = 0.0   # PD derivative memory
        self._escape_attempts = 0     # escalating override counter
        self._blind_frames    = 0     # consecutive visually-blind frames
        self._radar_sweep_dir = "turn_right"  # default sweep direction for escapes
    ...
    def _score_target_images(self, target_imgs):
        ...
        n_views_agree = sum(
            1 for _, q, _ in view_encodings
            if int(self.index.search(q, 10)[1][0][0]) == best_node
        )

        print(f"  [combined] Best goal node: {best_node} "
              f"(combined={best_combined:.4f}, views_agree={n_views_agree}/{len(view_encodings)})")

        if n_views_agree == 0:
            print("  [combined] ⚠️  No single view's top-1 match agrees — goal may be unreliable.")
    ...
    def _act_idle(self):
        ...
        if graph_action == "backward":
            ...
            if back_info and not back_info.get("blocked", False):
                ...
                self._prev_e_k = 0.0 # Reset PD state on manual override
                self.state             = "MOVING"
        ...
        self._prev_e_k = 0.0 # Reset PD controller state on new action
        self.state = "MOVING"
        self.alignment_history = []
        self.consecutive_low_alignment = 0
        self._escape_attempts = 0     # only reset on successful path commitment
    ...
    def _act_moving(self):
        ...
        if primary < 0.3:
            ...
            if self.consecutive_low_alignment > 3:
                ...
                self._prev_e_k = 0.0 # Reset PD state on alignment loss
                self.state = "IDLE"
                self.cached_direction_scores = None
                self.cached_alignment        = None
                return Action.IDLE
       ...
       # 5. Scan directions (cached)
        ...
        if "front" in self.cached_direction_scores:
            ...
            if front_distance > 1 and front_score > 0.8:
                ...
                self._prev_e_k = 0.0 # Reset PD state on new node
                self.state = "IDLE"
                self.cached_direction_scores = None
                self.cached_alignment        = None
                return Action.IDLE
        ...
        if self.navigator.detect_junction(self.cached_direction_scores, threshold=0.6):
            ...
            self._prev_e_k = 0.0 # Reset PD state on junction
            self.state = "IDLE"
            self.cached_direction_scores = None
            self.cached_alignment        = None
            return Action.IDLE
        ...
        if self._is_facing_corner(self.prev_fpv, self.fpv):
            ...
            self._prev_e_k = 0.0 # Reset PD state on corner
            self.state = "IDLE"
            self.cached_direction_scores = None
            self.cached_alignment        = None
            self.prev_fpv = self.fpv.copy() if self.fpv is not None else None
        ...
        if self.navigator.detect_dead_end(self.cached_direction_scores, threshold=0.4):
            ...
            self._prev_e_k = 0.0 # Reset PD state on dead end
            self.state = "IDLE"
            self.cached_direction_scores = None
            self.cached_alignment        = None
            return Action.IDLE
