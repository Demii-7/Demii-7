...
# Persistence
MAP_FOLDER = "/Users/demarcewilliams/Downloads/exploration_data/Final"
DATA_DIR   = "/Users/demarcewilliams/Downloads/exploration_data/Final/trajectory_data"             # root folder containing trajectory images
...
class AutoNavPlayer(Player):
    ...
    def pre_navigation(self):
        ...
        # 2. Identify target_node
        if self._kmeans is not None:
            self._target_node = localize_image(
                self._target_front_img, self._kmeans, self._faiss_idx,
    ...
    def act(self):
        ...
        if self._mode == "manual":
            ...
            self.last_act = Action.IDLE # Consume manual tap
            return action
    ...
    def _wander(self, L, C, R, mask, global_bias=None):
        ...
        if global_bias == "Left":
            ...
            R += BIAS_STRENGTH
        # (If the map says Forward, we don't need to boost C because Wander already prefers going straight)
        ...
        # 3. PRE-EMPTIVE TURN
        if total < 0.40: 
            # Because L and R are biased now, this will naturally pick the "correct" map direction!
   ...
   # ── EVASIVE planner ───────────────────────────────────────────────────────
    ...
    def _confirming_act(self):
        """
        Stage 1: VPR node match fired CMD_CHECKIN (background thread).
        Stage 2: SIFT match FPV vs saved target front image.
          >= MIN_MATCH_COUNT good matches -> fire CHECKIN
          fewer matches -> rotate right to align with target orientation
        Global commands are NOT read in this state.
        """
        if self.fpv is None or self._target_front_img is None:
            return Action.FORWARD

        # Use get_cached_features -- raw frames are not cached
        kp_q, des_q = get_cached_features(self.fpv)
        kp_t, des_t = get_cached_features(self._target_front_img)
        ...
        if len(good) >= MIN_MATCH_COUNT:
            print("!!! [SUCCESS] SIFT Match Confirmed. Firing CHECKIN. !!!")
            return Action.CHECKIN
        else:
            if self._frame_count % 20 == 0:
                print(f"--- [ALIGNING] SIFT Matches: {len(good)}/{MIN_MATCH_COUNT} - Rotating... ---")
            return Action.RIGHT
