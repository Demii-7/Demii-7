...
class MazeNavigator:
    ...
    def scan_directions(self, fpv: np.ndarray,
        ...
        """
        Determine which direction (front/left/right/back) leads toward the
        IMMEDIATE next node on the path.

        Compares all 4 directional crops against the single next node's
        descriptor. Whichever crop scores highest = that's where the
        next node is relative to the camera.

        Also checks the next 2-3 nodes to provide distance context.
        Total: 4 DINOv2 calls (one per direction crop).
        """
        ...
        # The IMMEDIATE next node is what matters for direction
        next_node = self.current_path[1]
        if next_node >= len(self.descriptors):
            return {}

        next_desc = self.descriptors[next_node].reshape(1, -1).astype(np.float32)

        # Also get descriptors for nodes 2-3 ahead for tiebreaking
        lookahead_descs = []
        for i in range(2, min(4, len(self.current_path))):
            n = self.current_path[i]
            if n < len(self.descriptors):
                lookahead_descs.append(
                    self.descriptors[n].reshape(1, -1).astype(np.float32)
                )

        h, w = fpv.shape[:2]
        pil = Image.fromarray(cv2.cvtColor(fpv, cv2.COLOR_BGR2RGB))

        # 4 directional crops including back
        crops = {
            "front": pil.crop((w // 6, h // 6, 5 * w // 6, 5 * h // 6)),
            "left":  pil.crop((0,      h // 6, w // 2,      5 * h // 6)),
            "right": pil.crop((w // 2, h // 6, w,           5 * h // 6)),
            "back":  pil.transpose(Image.Transpose.ROTATE_180).crop(
                         (w // 6, h // 6, 5 * w // 6, 5 * h // 6)),
        }
        ...
        for direction, crop in crops.items():
            ...
            # Primary: similarity to the IMMEDIATE next node
            primary_sim = float(np.dot(q, next_desc.T)[0, 0])

            # Secondary: average similarity to nodes 2-3 ahead (tiebreaker)
            lookahead_sim = 0.0
            if lookahead_descs:
                for ld in lookahead_descs:
                    lookahead_sim += float(np.dot(q, ld.T)[0, 0])
                lookahead_sim /= len(lookahead_descs)

            # 80% weight on immediate next, 20% on lookahead
            combined = 0.8 * primary_sim + 0.2 * lookahead_sim
            ...
            results[direction] = {
                "node":     next_node,
                "score":    combined if not is_blocked else combined * 0.3,
                "distance": 1,
                "blocked":  is_blocked,
                "primary":  primary_sim,
            }
