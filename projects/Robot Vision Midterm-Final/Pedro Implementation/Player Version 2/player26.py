...
class KeyboardPlayerPyGame(Player):
    ...
    def _act_moving(self):
        ...
        # 6. Continue on planned route (resolve use_radar)
        action_str = self.navigator.next_action()
        if action_str == "use_radar":
            action_str, detail = self._resolve_action(action_str, small_fpv)
            print(f"[moving] Resolved use_radar → {action_str} ({detail})")
        else:
            print(f"[moving] Next action (advisory): {action_str}")
    ...
    def _manual_update_and_display(self):
        ...
        resolves 'use_radar' into a physical direction, and shows the panel.
        """
        ...
        # 4. Get next action and resolve "use_radar"
        raw_action = self.navigator.next_action() if hops >= 1 else "stop"
        resolved_action, detail = self._resolve_action(raw_action, small_fpv)
    ...
    def _resolve_action(self, raw_action: str, small_fpv: np.ndarray) -> tuple:
        """
        Translate raw navigator action into a physical direction the player
        can actually execute. Resolves 'use_radar' by scanning crops against
        the next node on the path.

        Returns (action_str, human_detail).
        """
        if raw_action not in ("use_radar", "stop"):
            return raw_action, f"path says {raw_action}"
        ...
        # ── Resolve "use_radar": which physical direction matches the next node? ──
        path = self.navigator.current_path or []
        if len(path) < 2:
            return "forward", "use_radar → defaulting forward (no path)"

        next_node = path[1]
        if next_node >= len(self.navigator.descriptors):
            return "forward", "use_radar → defaulting forward (bad node)"

        # Compare directional crops against the next node's descriptor
        crops = self.navigator.extract_directional_crops(small_fpv)
        next_desc = self.navigator.descriptors[next_node].reshape(1, -1).astype(np.float32)
        ...
        for direction, crop in crops.items():
            if direction == "back":
                continue  # skip back — unlikely we need to go backwards
            with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp:
                tmp_path = tmp.name
            crop.save(tmp_path)
            try:
                q = self.navigator.encoder.encode(tmp_path).reshape(1, -1).astype(np.float32)
            finally:
                os.unlink(tmp_path)

            sim = float(np.dot(q, next_desc.T)[0, 0])
            if sim > best_score:
                best_score = sim
                best_dir = direction
        ...
        return resolved, f"loop-closure → look {best_dir} (sim={best_score:.2f})"
    ...
    def display_next_best_view(self):
        ...
        # Next action — use the already-resolved suggestion (no "use_radar")
        ...
        if len(path) >= 2:
            ...
            # Sample intermediate frame indices evenly between current and next
            if abs(next_fidx - cur_fidx) > 1:
                step_dir = 1 if next_fidx > cur_fidx else -1
                all_intermediate = list(range(cur_fidx + step_dir, next_fidx, step_dir))
                # Pick N_MICRO evenly spaced samples
                if len(all_intermediate) > N_MICRO:
            ...
            # Load and display each intermediate frame
            ...
            for i, fidx in enumerate(indices):
                ...
                label = f"step {i+1}/{len(indices)}"
                cv2.putText(img, label, (4, 16), FONT, 0.35, (255, 255, 255), 1, AA)
        ...
        cv2.putText(row2_label, "Micro-steps to next node (what you'll see walking)",
                    (6, 14), FONT, 0.38, (0, 200, 200), 1, AA)

        # ── Row 3: BROADER PATH — next keyframe nodes with edge actions ───
        N_PREVIEW = 5
        ...
        for p in range(N_PREVIEW):
            if p < len(preview_nodes):
                ...
                # Get the edge action from previous node to this node
                prev_node = path[p] if p < len(path) else None
                edge_action = ""
                if prev_node is not None:
                    ed = self.navigator.G.get_edge_data(prev_node, node_idx)
                    if ed:
                        ea = ed.get("action", "forward")
                        if ea == "loop" or ed.get("edge_type") == "loop_closure":
                            ea = "jump"
                        edge_action = ea.upper()

                cv2.rectangle(img, (0, 0), (PW-1, PH-1), (200, 200, 0), 1)
                cv2.putText(img, f"+{p+1} n{node_idx}", (4, 16),
                            FONT, 0.35, (255, 255, 255), 1, AA)
                if edge_action:
                    cv2.putText(img, edge_action, (4, PH - 8),
                                FONT, 0.38, (0, 255, 100), 1, AA)
            else:
                img = np.zeros((PH, PW, 3), dtype=np.uint8)
            cells.append(img)
        ...
        cv2.putText(row3_label, "Path ahead (keyframe nodes with actions)",
