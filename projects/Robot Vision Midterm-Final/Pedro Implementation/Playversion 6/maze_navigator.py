...
class MazeNavigator:
    ...
    def scan_directions(self, fpv: np.ndarray,
        ...
        Uses ONLY stored FAISS descriptors — no keyframe image re-encoding.
        Total: 3 DINOv2 calls (one per direction crop).
        """
        ...
        # 3 directional crops — no "back" (wastes an encoder call)
        crops = {
        ...
        for direction, crop in crops.items():
            # One encoder call per direction
            with tempfile.NamedTemporaryFile(suffix=".jpg", delete=False) as tmp:
                tmp_path = tmp.name
            crop.save(tmp_path)
            try:
            ...
            # Compare against stored descriptors only — fast dot products
            ...
            if best_node is not None:
                # Feasibility check — pure CV, no encoder calls
