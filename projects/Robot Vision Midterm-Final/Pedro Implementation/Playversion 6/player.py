...
AUTO             = True   # If True, player will autonomously navigate using the navigator. If False, player will idle and allow manual control (e.g., via keyboard).
...
class KeyboardPlayerPyGame(Player):
    ...
    def see(self, fpv):
        ...
        if self._goal_img_path is None:
            ...
            if best_path is None:
                ...
                return
            self._goal_img_path = best_path
            self._goal_node_direct = best_node  # store the node from combined scoring
        ...
        # Use the node found by combined scoring — don't re-search by single image
        ...
        # Save current FPV for the next optical-flow corner check
        self.prev_fpv = fpv.copy()
    ...
    def _score_target_images(self, target_imgs):
        """
        Score target images using ALL 4 views combined.
        
        Instead of picking the single best view, find the graph node that
        best matches across ALL views. This prevents one misleading view
        from sending us to the wrong goal.
        
        Returns (best_path, best_score, best_node) where best_path is the saved
        image of the best individual view (for display) and best_node is the
        graph node found by combined voting.
        """
        view_names = ["front", "left", "back", "right"]
        
        # Encode all 4 views and find their top candidates
        view_encodings = []
        view_paths = []
        ...
        # For each view, get top-10 FAISS matches and accumulate votes per node
        node_votes = {}  # node_idx → total score across all views
        ...
        # Find node with highest combined vote
        if not node_votes:
            return None, -1.0, None
        ...
        # Count how many views voted for this node (in top-10)
        n_views_agree = sum(
            1 for vname, q, _ in view_encodings
            for _, idx in zip(*self.index.search(q, 10))
            if int(idx[0]) == best_node  # check if best_node appears
        )
        
        print(f"  [combined] Best goal node: {best_node} "
              f"(combined={best_combined:.4f})")
        
        # Pick the view whose individual match to best_node is strongest (for display)
        best_view_path = None
        best_view_score = -1.0
    ...
    def _act_moving(self):
        ...
        # ── OPTICAL FLOW GEOMETRY CHECK ──
        if self._is_facing_corner(self.prev_fpv, self.fpv):
            print("[moving] 🔄 Optical Flow override! Spinning away from corner.")
            self.state = "IDLE"
            self.cached_direction_scores = None
            self.cached_alignment = None
    ...
    def _resolve_action(self, raw_action: str, small_fpv: np.ndarray,
        ...
        which compares the current front/left/right/back crops against the
        NEXT path node. Whichever direction best matches = that's where to go.
        ...
        if direction_scores:
            ...
            # Build detail showing ALL directions (including blocked)
            detail_parts = []
    ...
    def _simplify_path(self, path: list) -> list:
        ...
        Shows:
          - Turns, junctions, loop closures
          - Checkpoint every MAX_STRAIGHT hops on straight stretches
          - The goal
        
        This ensures waypoints are never too far apart even on long corridors.
        """
        ...
        goal = self.navigator._goal_node
        waypoints = []
        MAX_STRAIGHT = 5   # show a checkpoint at least every 5 hops
        last_waypoint_hop = 0
        ...
        for i in range(1, len(path)):
            ...
            if is_goal:
                ...
                waypoints.append({
                    "node": node, "hops_away": hops_away,
                    "type": "junction", "label": f"JUNCTION"
                })
                last_waypoint_hop = hops_away
            elif hops_since_last >= MAX_STRAIGHT:
                # Checkpoint on long straight stretches
                remaining = max(0, len(path) - 1 - hops_away)
                waypoints.append({
                    "node": node, "hops_away": hops_away,
                    "type": "checkpoint", "label": f"STRAIGHT ({remaining} left)"
                })
                last_waypoint_hop = hops_away
        ...
        # If no waypoints found (very short path), show the goal
        ...
        for p in range(N_WAYPOINTS):
            if p < len(waypoints):
                ...
                if wp["type"] == "turn":
                    ...
                    border_color = (200, 100, 255)  # purple for warps
                elif wp["type"] == "goal":
                    border_color = (0, 255, 0)      # green for goal
                elif wp["type"] == "junction":
                    border_color = (0, 255, 255)    # yellow for junctions
                elif wp["type"] == "checkpoint":
                    border_color = (180, 180, 180)  # gray for checkpoints
                else:
                    border_color = (200, 200, 0)    # default
